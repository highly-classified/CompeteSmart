...
import { Target, TrendingUp, AlertTriangle, Lightbulb, CheckCircle2, RefreshCw, ArrowLeft } from "lucide-react";
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-20">
            ...
            {experiments.slice(0, 6).map((exp, index) => {
              ...
              const glowColor = risk < 0.3 ? "bg-emerald-500/5" : risk < 0.7 ? "bg-amber-500/5" : "bg-red-500/5";
              const riskBadgeClasses = risk < 0.3 ? "bg-emerald-500/10 text-emerald-300 border-emerald-500/20" : risk < 0.7 ? "bg-amber-500/10 text-amber-300 border-amber-500/20" : "bg-red-500/10 text-red-300 border-red-500/20";
              const linkClasses = risk < 0.3 ? "text-emerald-400" : risk < 0.7 ? "text-amber-400" : "text-red-400";
              const displayName = exp.cluster_id.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
              ...
              return (
                ...
                >
                  ...
                  {/* Layer Tags */}
                  <div className="flex flex-wrap gap-2 mb-6">
                    <span className="text-[9px] px-2 py-1 rounded-md bg-white/5 text-zinc-400 font-medium tracking-tight">Intelligence Layer</span>
                    <span className="text-[9px] px-2 py-1 rounded-md bg-white/5 text-zinc-400 font-medium tracking-tight">Decision Layer</span>
                  </div>
